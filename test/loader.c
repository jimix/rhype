	     ...
	     uval istart, uval isize)
{
	...
	uval ea;
	uval size;
	ra = ALIGN_DOWN(ps->mem[0].addr + offset, 1 << LOG_CHUNKSIZE);
	size = ALIGN_UP(ps->mem[0].addr + offset - ra + isize, PGSIZE);
		 ...
		 uval ofd)
{
	...
	rc = hcall_create_partition(ret, ps->mem[0].addr, ps->mem[0].size,
				    magic);

	/* This is what the new partition sees it's memory as */
	ps->pmem[0].addr = 0;
	ps->pmem[0].size = ps->mem[0].size;
	...
	for (i = 0; i < segno; i++) {
		...
		if (le->file_size > 0) {
			hprintf("%s: copying bits from %s: to 0x%lx[0x%lx]\n",
				__func__, ps->name,
				ps->mem[0].addr + offset + load_offset,
				isize);
...
launch_image(uval init_mem, uval init_mem_size, const char* name, uval ofd)
{
	...
	for (i = 0; i < MAX_MANAGED_PARTITIONS; ++i) {
		if (partitions[i].active == 0) {
			...
			partitions[i].lpid = 0;
			partitions[i].mem[0].addr = init_mem;
			partitions[i].mem[0].size = init_mem_size;
...
reload_image(const char* name, uval ofd)
{
	...
	int i;
	uval total;
	uval max = CHUNK_SIZE * CONTROLLER_MEM_CHUNKS;

	ps->lpid = H_SELF_LPID;

	memcpy(ps->mem, &pinfo[1].mem[0],
	       MAX_MEM_RANGES * sizeof(struct mem_range));
	memcpy(ps->pmem, &pinfo[1].mem[0],
	       MAX_MEM_RANGES * sizeof(struct mem_range));

	total = 0;
	for (i = 0; i < MAX_MEM_RANGES; ++i) {
		if (total >= max) {
			ps->mem[i].addr = ps->mem[i].size = 0;
			ps->pmem[i].addr = ps->pmem[i].size = 0;
			continue;
		}

		if (total + ps->mem[i].size > max) {
			ps->mem[i].size = (max - total);
			ps->pmem[i].size = (max - total);
		}
		total += ps->pmem[i].size;
	}
	...
	for (i = 0; i < segno; i++) {
		le = &(lf.loaderSegmentTable[i]);
		uval start = le->va_start;
		if (start < begin) {
			begin = start;
		}

		if (start + le->file_size > end) {
			end = start + le->file_size;
		}
	}
	...
	{
		if (begin == 0) {
			begin = 0xc000000000000000UL;
		}

		uval class_base = ALIGN_DOWN(begin,
					     1ULL << VM_CLASS_SPACE_SIZE_BITS);
		hcall_create_vm_class(NULL, H_VM_CLASS_LINEAR, 1, class_base,
				      1ULL << VM_CLASS_SPACE_SIZE_BITS, 0,
				      0, 0, 0);
