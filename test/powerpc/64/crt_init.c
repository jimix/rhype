...
union slb_entry cache[SWSLB_SR_NUM];
...
map_pages(uval raddr, uval eaddr, uval size)
{
	...
	l = 0;
	lg = 0;
	slbe = slb_insert(eaddr, 0, 0, 1, 0, cache);
#else
	slbe = slb_insert(eaddr, 1, SELECT_LG, 1, 0, cache);
#endif
...
#ifdef FORCE_APPLE_MODE
    if (msr & MSR_AM) {
	    hcall_set_spr(NULL, H_MSR, msr | MSR_SF);
	    os_wrapper(r3, r4, r5, r6, msr);
    } else
