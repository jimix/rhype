...
ofd_root_props(void *m, uval lpid)
{
	...
	if (lpid == iohost_lpid) {
		/* fi not defined, just leave it alone */
#ifdef MACHINE_NAME_STRING
...
ofd_openpic_owner_props(void *m, uval lpid)
{
	...
	if (addr == ~0ULL) {
		static const char mac_k2_openpic[] = "/ht/pci@3/mac-io/mpic";
		ofdn_t p = ofd_node_find(m, mac_k2_openpic);
		if (p < 0) {
			return;
		}
		/* We're on a mac.  There is a way to calculate this,
...
ofd_openpic_probe(void *m, uval little_endian)
{
	...
	save_mmio_range((uval)addr, 0x40000, "OpenPIC");

	hprintf("OpenPIC at: %lx\n", addr);
...
ofd_memory_props(void *m, struct mem_range partition_mem[MAX_MEM_RANGES])
{
	...
	do {
		...
		ofd_getcells(m, old, &cells, &size_cells);
	...
	for (i = 0; i < MAX_MEM_RANGES; (mem[i].size == 0) ? ++i : i) {
		...
		if (start == 0) {
			...
			if (size_cells == 2) {
				regs[cells] = 0;
				regs[cells + 1] = CHUNK_SIZE -  (1 << 20);
			} else {
				regs[cells] = CHUNK_SIZE -  (1 << 20);
			}
		...
		if (cells == 2) {
			regs[0] = start >> 32;
			regs[1] = start & 0xffffffffULL;
		} else {
			regs[0] = start & 0xffffffffULL;
		}
