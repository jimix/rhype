...
#ifdef HVAL_IS_64
#define HVAL_CHOOSE(x32, x64) x64
#define HVAL UVAL_CHOOSE("0x%016llx", "0x%016lx")
#else
#define HVAL_CHOOSE(x32, x64) x32
#define HVAL"0x08lx"
#endif
...
#define HCALL_FAIL_CHECK(ret, retval)					\
	ASSERT(ret >= 0 && retval == 0,					\
	       "hcall failure: %d " HVAL "\n", ret, retval)

#define HCALL_FAIL_CHECK_MSG(ret, retval, msg)				\
	ASSERT(ret >= 0 && retval == 0,					\
	       "hcall failure (%s): %d " HVAL "\n", msg, ret, retval)
...
extern int parse_laddr(const char* arg, hval *base, hval *size);
extern hval mem_hold(hval size);
