...
extern void __gdbstub encode_to_packet(const char *str, sval len);

extern void __gdbstub send_packet(void);
extern void __gdbstub reset_out_buf(void);
...
extern void __gdbstub gdb_write_register(struct cpu_state *state,
					 int id, char *buf)
	__attribute__ ((weak));

extern void __gdbstub gdb_read_register(struct cpu_state *state, int id)
	__attribute__ ((weak));


struct gdb_monitor_task
{
	struct gdb_monitor_task* gdb_mt_next;
	const char* gdb_mt_name;
	uval (*gdb_mt_fn)(const char *cmd);
};
...
struct gdb_functions
{
	...
	uval (*write_to_mem)(struct cpu_state *state, char *mem,
			     uval len, const char *buf);
	void (*read_register)(struct cpu_state *state, int id);
	void (*write_register)(struct cpu_state *state, int id, char* buf);
	struct gdb_monitor_task *monitor_list;
};

extern struct gdb_functions gdb_functions;
...
extern uval __gdb_debug;
#define gdb_debug(ARGS...) do { if (__gdb_debug) hprintf(ARGS); } while (0);
