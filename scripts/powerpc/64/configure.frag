...
echo $ac_n "checking whether to build for Apple Mode""... $ac_c" 1>&6
echo "configure:638: checking whether to build for Apple Mode" >&5
# Check whether --enable-apple_mode or --disable-apple_mode was given.
if test "${enable_apple_mode+set}" = set; then
  enableval="$enable_apple_mode"
  if test "$enableval" = "yes" ; then
	APPLE_MODE="yes";
	cat >> confdefs.h <<\EOF
#define FORCE_APPLE_MODE 1
EOF
...
echo "$ac_t""${APPLE_MODE}" 1>&6
ENABLE_APPLE_MODE=${APPLE_MODE}

if test "$ENABLE_APPLE_MODE" = "yes"; then
  PLUGINDIRS="${PLUGINDIRS} gdb"
  opt_outputs="${opt_outputs} plugins/gdb/Makefile"
  opt_outputs="${opt_outputs} plugins/gdb/powerpc/Makefile"
  opt_outputs="${opt_outputs} plugins/gdb/powerpc/64/Makefile"
fi
