...
#define LLAN_BL_NUM_POOLS	8
struct buffer_list
{
	union tce_bdesc bl_entry[LLAN_BL_NUM_POOLS+1];
	union tce_bdesc __unused[(PGSIZE/sizeof(union tce_bdesc))-
				 LLAN_BL_NUM_POOLS-1-1];
	uval	statistics;
};

/*
 * Each buffer that is added via h_add_logical_lan_buffer has stored
 * in it a linked-list entry.  The first 8 bytes are reserved, but the
 * RPA allows us to use the rest of the buffer as we see fit.
 * The "pointers" in this linked list are actually tce_bdesc's so it's
 * safe to expose them in OS accessible buffers.
 */
struct pool_list_entry
{
	uval64		ple_reserved;
	union tce_bdesc	ple_next;
};
