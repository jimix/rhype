...
main(void)
{
	...
	DECLARE(TCA_VREGS, offsetof(struct thread_control_area, vregs));

	DECLARE(TCA_SRR0, offsetof(struct thread_control_area, srr0));
	DECLARE(TCA_SRR1, offsetof(struct thread_control_area, srr1));

	DECLARE(TCA_SAVE_AREA,
		offsetof(struct thread_control_area, save_area));

	DECLARE(TCA_RESTORE_FN,
		offsetof(struct thread_control_area, restore_fn));
	DECLARE(TCA_VSTATE, offsetof(struct thread_control_area, vstate));
