...
{
	union slb_entry *e = curThread->slb_entries;
	int i;
...
{
	union slb_entry *slb_entry = thread->slb_entries;
	int i;

	/* save all extra SLBs */
	for (i = 0; i < SWSLB_SR_NUM; i++) {

		get_slb_entry(i, slb_entry);
#ifdef SLB_DEBUG
...
restore_slb(struct cpu_thread *thread)
{
	...
	/* restore all extra SLBs */
	for (i = 0; i < SWSLB_SR_NUM; i++) {
		inval_slb_entry(i, slb_entry);

		load_slb_entry(i, slb_entry);
