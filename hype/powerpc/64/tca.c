...
save_sprs(struct cpu_thread *thread)
{
	if (hv_features(thread->cpu->os)) {
		...
		thread->reg_tb = mftb();
		thread->reg_dec = thread->vregs->v_dec;

		thread->reg_sprg[0] = thread->vregs->v_sprg0;
		thread->reg_sprg[1] = thread->vregs->v_sprg1;
		thread->reg_sprg[2] = thread->vregs->v_sprg2;
		thread->reg_sprg[3] = thread->vregs->v_sprg3;

		thread->reg_dar = thread->vregs->v_dar;
		thread->reg_dsisr = thread->vregs->v_dsisr;
...
restore_sprs(struct cpu_thread *thread)
{
	...
	if (hv_features(thread->cpu->os)) {
		...
		thread->vregs->v_dec = thread->reg_dec;

		thread->vregs->v_sprg0 = thread->reg_sprg[0];
		thread->vregs->v_sprg1 = thread->reg_sprg[1];
		thread->vregs->v_sprg2 = thread->reg_sprg[2];
		thread->vregs->v_sprg3 = thread->reg_sprg[3];

		thread->vregs->v_dar = thread->reg_dar;
		thread->vregs->v_dsisr = thread->reg_dsisr;
#endif /* FORCE_APPLE_MODE */
	}
}
...
preempt_thread(struct cpu_thread *thread, uval timer)
{
	 ...
	 * it should be decremented by.	 */
	assert(!(tca->vstate & VSTATE_ACTIVE_DEC),
...
#error "this does not work yet"
	...
	tca->cca = cca;
	tca->primary_tca = tca;
	tca->hypeStack = stack | 0xf000000000000000ULL;
	tca->cached_hca = hca;
	tca->hypeToc = get_toc();
