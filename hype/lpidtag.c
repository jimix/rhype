...
lpidtag_acquire(struct os *os)
{
	...
	lpidtag_os_map[os->po_lpid_tag] = os;
...
{
	lpidtag_os_map[os->po_lpid_tag] = NULL;
...
lpidtag_init(int num_lpidr_bits)
{
	...
	lpidtag_os_map = halloc(sizeof(struct os*) << num_lpidr_bits);
	memset(lpidtag_os_map, 0, sizeof(struct os*) << num_lpidr_bits);
}
