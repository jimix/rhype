...
#define POOL_SIZE_MATCH	1
#define POOL_INIT	2
static union tce_bdesc *
llan_bdesc_pool_head(struct llan *l, uval sz, uval flags)
{
	uval i;
	uval empty = 0;
	union tce_bdesc td;
	...
	i = LLAN_BL_POOL_START;
	while (i < LLAN_BL_COUNTER) {
		copy_in(&td, &l->ll_blist[i], sizeof(td));

		if (td.lbd_dword == 0 && !empty) {
			empty = i;
		}
		if ((flags & POOL_SIZE_MATCH) && td.lbd_bits.lbd_len == sz) {
			return &l->ll_blist[i];
		} else if (td.lbd_bits.lbd_len >= sz) {
			return &l->ll_blist[i];
		}
		++i;
	}
	if (empty && (flags & POOL_INIT)) {
		i = empty;
		td.lbd_dword = 0;
		td.lbd_bits.lbd_len = sz;
		copy_out(&l->ll_blist[i], &td, sizeof(td));
		return &l->ll_blist[i];
	}
...
llan_add_buf(struct os *os, uval uaddr, union tce_bdesc bd)
{
	...
	pool = llan_bdesc_pool_head(l, bd.lbd_bits.lbd_len,
				    POOL_SIZE_MATCH | POOL_INIT);

	/* FIXME: this is probably true */
	assert(pool != NULL,
	       "simple pool indexing is insufficient\n");

	/* Insert ple as first element of list anchored at pool */
	struct lbd_bits_s lbd_bits;
	copy_in(&lbd_bits, pool, sizeof (lbd_bits));
	copy_out(&ple->ple_next, &lbd_bits, sizeof(lbd_bits));
	copy_out(pool, &bd, sizeof(bd));
...
static void *
llan_getbuf(struct llan *l, uval size, uval flags)
{
	union tce_bdesc *ptr = NULL;
	union tce_bdesc head;
	union tce_bdesc next;
	struct pool_list_entry *ple = NULL;

	lock_acquire(&l->ll_lock);

	ptr = llan_bdesc_pool_head(l, size, flags);

	assert(ptr, "head is an invalid buffer reference\n");
	if (ptr) {
		copy_in(&head, ptr, sizeof(head));
		ple = llan_bd_tce_xlate(l, head);
	}

	if (ple) {
		copy_in(&next, &ple->ple_next, sizeof(next));
		copy_out(ptr, &next, sizeof(next));
	} else {
		uval64 w;
...
llan_freebuf(struct os *os, uval uaddr, uval size)
{
	...
	rb = llan_getbuf(l, size, POOL_SIZE_MATCH);
	if (rb == NULL) {
		return H_Not_Found;
	}
...
#error "CACHE_LINE_SIZE is not defined"
	...
	rb = llan_getbuf(l, sum + align, 0);	/* includes the opaque word */
	if (rb == NULL) {
		return H_Busy;
	}
