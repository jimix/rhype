...
struct thread_control_area {
	...
	sval32 hdec;
	struct vregs* vregs;
	struct vexc_save_regs* save_area;
	void (*restore_fn)(void); /* optional code for register restore
...
#ifdef FORCE_APPLE_MODE
static inline void
sync_thread_to_tca(struct cpu_thread *thr, struct thread_control_area *tca)
{
	tca->vregs = thr->vregs;
}
